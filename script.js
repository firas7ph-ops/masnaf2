...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Sections with dark backgrounds where nav should stay light-colored
  const darkSections = ['home', 'our-story', 'radaim-blog'];
  ...
  const updateNav = () => {
    ...
    if (!scrolled) {
      // Check if we're over a dark-bg section
      let overDark = false;
      darkSections.forEach(id => {
        const el = document.getElementById(id);
        if (!el) return;
        const rect = el.getBoundingClientRect();
        if (rect.top <= 40 && rect.bottom >= 40) overDark = true;
      });
  ...
  if (toggle && navLinks) {
    toggle.addEventListener('click', () => {
      ...
      nav.classList.toggle('menu-open', isOpen);   // force solid bg on nav
